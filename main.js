...
document.querySelectorAll('.has-dropdown').forEach(li => {
 ...
 // Click on touch devices / keyboard nav
...
// Contact form anti-spam / validation
const contactForm = document.querySelector('form[data-contact-form]');
if (contactForm) {
 const startedAt = contactForm.querySelector('#form-started-at');
 const honeypot = contactForm.querySelector('#company-website');
 const messageField = contactForm.querySelector('#message');
 const emailField = contactForm.querySelector('#email');
 const nameFields = Array.from(contactForm.querySelectorAll('#fname, #lname'));
 ...
 const isLikelySpam = () => {
 const started = parseInt(startedAt?.value || '0', 10);
 const filledTooFast = started && Date.now() - started < minFillTimeMs;
 const trapFilled = Boolean(honeypot?.value.trim());
 return trapFilled || filledTooFast;
 };
 ...
 if (isLikelySpam()) {
 showToast('We could not verify this submission. Please wait a moment and try again.');
 return;
 }

 const invalidName = nameFields.some(field => {
 ...
 if (invalidName || message.length < 20) {
 showToast('Please complete all required fields with a bit more detail.');
 return;
 }
